...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Import Data Kelas</h6>
        </div>
        <div class="card-body mb-4">
            <form action="<?php echo base_url("admin/data-kelas/import") ?>" method="POST" class="mb-4" enctype="multipart/form-data">
                <div class="row">
                    <div class="offset-4 col-md-4">
                        <label for="nama_kelas" class="form-control-label">Sampel Import Data</label>
                        <br>
                        <a href="<?php echo base_url("admin/download-template/kelas") ?>" class="btn btn-success btn-block">
...
if (isset($_POST['submit'])) {
    ...
    if(!empty(trim($file_name)) && !empty(trim($temporary))) {
        if(in_array($file_type, $file_mimes)) {
            ...
            $insert= execute("INSERT INTO kelas (nama_kelas, kelas, jurusan) VALUES $values");

            if($insert) {
                unlink("../import/".$file_name);
                alert("Data kelas berhasil diimport!", base_url("admin/data-kelas"));
            } else {
                alert("Data kelas gagal diimport!", "back");
            }
